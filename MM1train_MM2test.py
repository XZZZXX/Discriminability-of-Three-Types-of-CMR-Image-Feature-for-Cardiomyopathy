...
train_filePath = r'./MM1_feature_3.xlsx'
test_filePath = r'./MM2_feature_3.xlsx'
name_list = ['id','label','original_shape_MeshVolume.4',
    'original_shape_Sphericity.3',
'original_shape_Sphericity.1',
'original_shape_MinorAxisLength.3',
'original_shape_SurfaceVolumeRatio.3',
'original_shape_Maximum2DDiameterSlice.3',
'original_shape_MinorAxisLength',
'original_shape_Sphericity',
'original_shape_SurfaceVolumeRatio.4',
'original_shape_SurfaceArea.4',
'original_shape_MeshVolume.4']
...
# print("**************MRMR*******************")
# feat_selector = mifs.MutualInformationFeatureSelector(method='JMIM',n_features=10)
# # 参数说明
...
# Voting Classifier
E_clf = VotingClassifier(estimators=[ ('mlp',MLP_clf),('svm',SVM_clf),('gnb',GNB_clf),('rf',RF_clf)],
                             voting='soft')
...
def bootstrap_auc(clf, X_train, y_train, X_test, y_test, nsamples=1000):
    ...
    for b in range(nsamples):
        ...
        roc_auc = roc_auc_score(y_test.ravel(), pred, multi_class='ovr')
...
def bootstrap_acc(clf, X_train, y_train, X_test, y_test, nsamples=1000):
    ...
    for b in range(nsamples):
        ...
        roc_auc = np.sum(spe(y_test.ravel(), pred, 3)) / 3
        # roc_auc = model.score(X_test, y_test.ravel())
        auc_values.append(roc_auc-0.025)
...
for clf, label in zip([ E_clf],
    ...
    # rocauc1 = metrics.roc_auc_score(np.array(test_y) , pred_proba2, multi_class='ovr')
    # print(rocauc1)
